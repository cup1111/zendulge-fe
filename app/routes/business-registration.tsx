...
export default function BusinessRegistration() {
  ...
  return <BusinessRegistrationFlow step={step} formData={formData} onInputChange={handleInputChange} onSubmit={handleSubmit} onNext={nextStep} onPrev={prevStep} />;
}

function BusinessRegistrationFlow({ step, formData, onInputChange, onSubmit, onNext, onPrev }: any) {
  const sections = [
      ...
      <div className="bg-blue-100 border-b border-blue-200 p-4">
        <div className="max-w-4xl mx-auto">
          <p className="text-blue-800 text-sm">
            Business Registration Process - {currentSection?.title} ({step} of {maxSteps})
          </p>
        </div>
      </div>
      ...
      <div className="max-w-4xl mx-auto px-4 py-8">
        <div className="bg-white rounded-lg shadow-md p-8">
          ...
          <div className="flex justify-between mt-8">
            ...
            <button onClick={onSubmit} className="px-6 py-2 bg-shadow-lavender text-white rounded-lg hover:bg-shadow-lavender/90">
                Submit
              </button>}
          </div>
        </div>
